...
module.exports = class accounts_dao extends require('../model/accounts_mod'){
  ...
  static async login(req,res){
    let body = req.body
    let data = await this.loginUser(body.id,body.pwd,body.identity)
    if (data.length > 0){
  ...
  static async getAccountList(req,res){
    ...
    let total = accountList.length // total记录数据库中的账号总数
    // 如果能够获取账号列表,返回状态码200并返回获取到的数据,否则返回状态码404
    if (total > 0){
      let start = pageSize * (pageNum - 1)
      let end = Math.min(start + pageSize, total)
      let data = JSON.parse(JSON.stringify(accountList.slice(start, end)))
      res.status(200).json({
        "data": data,
        "total": total
      })
  ...
  static async addAccount(req,res) {
    let body = req.body
    await this.createAccount(body.id,body.pwd,body.identity).then(() => {
      res.status(201).end()
    }).catch(err => {
      res.status(403).end()
    })
  ...
  static async updateAccount(req,res) {
    let body = req.body
    await this.updateAccountById(req.params.id,body.pwd).then(result => {
      res.status(201).end()
    }).catch(err => {
      res.status(403).end()
    })
