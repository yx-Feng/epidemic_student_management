    ...
    <el-dialog title="修改个人资料" v-model="modifyDialogVisible" width="40%" @close="showProfile">
      <el-form v-for="(item,i) in labelList" :key="i" :model="personInfo"
        ...
        <el-form-item :label="item" :prop="propList[i]">
          <!-- 学号和身份不可修改 -->
          <el-input v-if="item === '学号' || item === '身份'" v-model="personInfo[propList[i]]" disabled></el-input>
...
export default {
  data () {
    return {
      ...
      identity: '', // 当前用户的身份
      // 存放个人资料
      personInfo: { id: '', name: '', sex: '', identity: '', class: '', college: '', tel: '', pwd: '' },
      modifyDialogVisible: false, // 控制修改个人资料对话框是否可见
      // 个人资料中的关键字
      labelList: ['学号', '姓名', '性别', '身份', '班级', '学院', '联系方式', '账号密码'],
      propList: ['id', 'name', 'sex', 'identity', 'class', 'college', 'tel', 'pwd'],
      // identity和身份的映射
  ...
  methods: {
    ...
    async showProfile () {
      try {
        const { data: res } = await this.$http.get('/pi/stu/' + this.id)
    ...
    modifyProfile () {
      this.$refs.modifyFormRef[0].validate(async valid => {
        if (!valid) return
        try {
          await this.$http.put('/pi/stu/' + this.id, {
