<template>
  <div>
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>审批通过</el-breadcrumb-item>
    </el-breadcrumb>
    <!-- 卡片视图区域 -->
    <el-card>
      <!-- 搜索与添加区域 -->
      ...
      <el-table :data="leaveList" border stripe style="width: 100%">
        ...
        <el-table-column label="不允许" width="100px">
          <template slot-scope="scope">
            <el-switch v-model="scope.row.state_no" @change="stateChange(scope.row)"></el-switch>
          </template>
        </el-table-column>
      </el-table>
      <!-- 分页区域 -->
...
export default {
  ...
  methods: {
    ...
    async getLeaveFormList () {
      const { data: res } = await this.$http.get('/leaveforms/coun/' + this.id + '/2', {
    ...
    async stateChange (leaveFormInfo) {
      const { data: res } = await this.$http.put(
        `leaveforms/${leaveFormInfo.s_id}/${leaveFormInfo.createdTime}/state/${!leaveFormInfo.state_no}`
      )
